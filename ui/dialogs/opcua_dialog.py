...
from ..theme import FORM_FIELD_STYLE, SPACING, MARGIN_H, MARGIN_V, FORM_MAX_WIDTH
import psutil
...
class OPCUADialog(QDialog):
    ...
    def _populate_adapters(self):
        ...
        try:
            ...
            # Do not add a 'Default' placeholder; present actual adapters only
            ...
            try:
                ...
                if target:
                    ...
                    if idx >= 0:
                        ...
                        try:
                            name = vals.get('network_adapter') or ''
                            display = f"{name} ({target})" if name else f"Auto ({target})"
                            na_widget.addItem(display, target)
                            # select the newly added item
                            idx2 = na_widget.findData(target)
                            if idx2 >= 0:
