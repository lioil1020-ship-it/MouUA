...
from ui.components import setup_table, to_numeric_flag, safe_data, call_controller, schedule_temp_export, collect_selected_tree_items
...
try:
    ...
    DataBroker = None
try:
    from core.OPC_UA import OPCServer
except Exception:
    try:
        from core.OPC_UA.OPC_UA import OPCServer
    except Exception:
        try:
...
class IoTApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.splitter.splitterMoved.connect(lambda pos, idx: setattr(self, '_splitter_user_moved', True))
        ...
        self.monitor_table.setSelectionMode(QTableWidget.SelectionMode.ExtendedSelection)
        ...
        self.monitor_table.verticalScrollBar().valueChanged.connect(self._on_monitor_scroll)
                        ...
                        if conn_node is not None:
                            for i in range(conn_node.childCount()):
                                ...
                                try:
                                    ...
                                    pass
                        if missing_params and self._last_project_file and os.path.exists(self._last_project_file):
                            try:
                                # 嘗試匯入上次儲存的專案，該檔案可能包含較完整的資料
                                self._call_controller('import_project_from_json', self._last_project_file)
        ...
        try:
            ...
            QTimer.singleShot(0, lambda: self.update_right_table(self.tree.conn_node, 0))
    ...
    def _ensure_initial_splitter_sizes(self):
        ...
        try:
            if not getattr(self, '_splitter_user_moved', False):
                ...
                self._set_splitter_sizes([left_calc, right_calc], reason='ensure_initial')
            else:
                half = int(total_w) // 2
                left_w = max(150, half)
                w_req = max(150, int(total_w) - left_w)
                self._set_splitter_sizes([left_w, w_req], reason='ensure_initial-fallback')
        except Exception:
            pass
    ...
    def show_terminal_window(self, device_item=None):
        ...
        if TerminalWindow is None:
            try:
                QMessageBox.warning(self, "Diagnostics Unavailable", "TerminalWindow component is missing.")
            except Exception:
                pass
            return None
    ...
    def open_opcua_settings(self):
        ...
        # Ensure settings are canonicalized before populating the dialog
        try:
            if initial is not None and hasattr(self, 'apply_opcua_settings'):
                try:
                    self.apply_opcua_settings(initial)
                except Exception:
                    pass
            initial = getattr(self, 'opcua_settings', None)
        except Exception:
                ...
                try:
                    ...
                    try:
                        if getattr(self, '_temp_json', None):
                            QTimer.singleShot(50, lambda: self._call_controller('export_project_to_json', self._temp_json))
                    except Exception:
                        pass
                except Exception:
                    pass
    ...
    def apply_opcua_settings(self, settings):
        ...
        """
        try:
            # received settings (no terminal output)
            # normalize via controller if present
            try:
                if getattr(self, 'controller', None) and hasattr(self.controller, 'normalize_opcua_settings'):
                    try:
            ...
            try:
                ...
                gen = {}
            try:
                auth = settings.get('authentication') if isinstance(settings.get('authentication'), dict) else {}
            except Exception:
                auth = {}
            try:
                sec = settings.get('security_policies') if isinstance(settings.get('security_policies'), dict) else {}
            except Exception:
                sec = {}
            try:
                cert = settings.get('certificate') if isinstance(settings.get('certificate'), dict) else {}
            except Exception:
                cert = {}
            ...
            nested = {
                ...
                'authentication': {
                    'authentication': auth.get('authentication') or auth.get('type') or 'Anonymous',
                    'username': auth.get('username', ''),
                    'password': auth.get('password', ''),
                },
                'security_policies': {k: bool(v) for k, v in (sec.items() if isinstance(sec, dict) else {})},
                'certificate': {**cert, 'auto_generate': bool(cert.get('auto_generate', True)), 'common_name': cert.get('common_name', '')},
            }
            ...
            try:
                ...
                if 'network_adapter_ip' not in flat and nested.get('general'):
                    flat['network_adapter_ip'] = nested['general'].get('network_adapter_ip', '')
            except Exception:
                pass
            ...
            try:
                if OPCServer is not None:
                    ...
                    # OPCServer implementation not available; no terminal output
                    pass
            except Exception:
                pass
        except Exception:
                ...
                for di in range(ch.childCount()):
                    ...
                    try:
                        if self._safe_data(dev, 0) == 'Device':
                            ...
                            act.triggered.connect(lambda checked=False, d=dev: self.show_terminal_window(d))
                            menu.addAction(act)
                    except Exception:
                        pass
        except Exception:
    ...
    def on_edit_item(self, item):
        ...
        if node_type == "Tag":
            try:
                ...
                pass
        QMessageBox.information(self, "Edit", "Edit action not available in this UI build")
    ...
    def on_edit_channel(self, item):
        ...
        try:
            ...
            if isinstance(driver_val, dict):
                ...
                drv_params = driver_val.get('params') if isinstance(driver_val.get('params'), dict) else (driver_val.get('params') or {})
            else:
                drv_type = driver_val
        ...
        dlg = ChannelDialog(self, suggested_name=(data.get("general", {}).get("name") or "Channel1"))
        try:
            dlg.load_data(data)
        except Exception:
            pass
    ...
    def on_edit_device(self, item):
        ...
        try:
            ...
            driver = "Modbus RTU Serial"
            data = {"general": {"name": item.text(0) or ""}}
        dlg = DeviceDialog(self, suggested_name=(data.get("general", {}).get("name") or "Device1"), driver_type=driver)
        try:
            dlg.load_data(data)
        except Exception:
            pass
    ...
    def on_edit_tag(self, item):
        ...
        dlg = TagDialog(self, suggested_name=data.get("general", {}).get("name") or "Tag1", suggested_addr=suggested_addr or "400000", is_new=False)
        try:
            dlg.load_data(data)
        except Exception:
            pass
    ...
    def on_edit_group(self, item):
        ...
        if item is None:
            return
        try:
            data = {"general": {"name": item.text(0) or "", "description": self._safe_data(item, 3, "") or ""}}
        except Exception:
            data = {"general": {"name": item.text(0) or "", "description": ""}}
        from ui.dialogs.group_dialog import GroupDialog

        suggested = (data.get("general") or {}).get("name") or data.get("name") or "Group1"
        ...
        if dlg.exec():
            ...
            try:
                if getattr(self, 'controller', None):
                    ...
                    item.setData(1, Qt.ItemDataRole.UserRole, new.get("description") or "")
            except Exception:
                try:
                    item.setText(0, new.get("name") or item.text(0))
                    item.setData(1, Qt.ItemDataRole.UserRole, new.get("description") or "")
        ...
        try:
            if _prev_sizes is not None and not getattr(self, '_splitter_user_moved', False):
    ...
    def on_delete_item(self, item):
        ...
        try:
            ...
            label = ""
        if QMessageBox.question(self, "Delete", f"確定刪除 '{label}'?") == QMessageBox.StandardButton.Yes:
            try:
    ...
    def on_paste_item(self, target_item):
        ...
        try:
            try:
                ttype = self._safe_data(target_item, 0, None) if target_item is not None else None
    ...
    def on_import_device_csv(self, device_item):
        ...
        try:
            ...
            path, _ = QFileDialog.getOpenFileName(self, "Import Device CSV", "", "CSV Files (*.csv);;All Files (*)")
            if not path:
                return
            ...
            try:
                ...
                QMessageBox.warning(self, "Import", f"匯入失敗，請檢查 CSV 格式: {str(e)}")
                return
    ...
    def on_export_device_csv(self, device_item):
        ...
        try:
            from PyQt6.QtWidgets import QFileDialog, QMessageBox
            path, _ = QFileDialog.getSaveFileName(self, "Export Device CSV", "device_export.csv", "CSV Files (*.csv);;All Files (*)")
            if not path:
                return
            try:
                # ensure extension
    ...
    def on_table_cell_double_clicked(self, row, _column):
        ...
        QMessageBox.information(self, "Open", f"Double-click on row {row} (detail view not implemented in UI-only build)")
    ...
    def on_table_context_menu(self, pos):
        ...
        try:
            ...
            if node_type == 'Connectivity':
                menu.addAction("➕ 新增 Channel", lambda: self.tree.request_new_channel.emit(current_node))
                menu.addSeparator()
            elif node_type == 'Channel':
                menu.addAction("➕ 新增 Device", lambda: self.tree.request_new_device.emit(current_node))
                menu.addSeparator()
            elif node_type in ('Device', 'Group'):
                menu.addAction("➕ 新增 Tag", lambda: self.tree.request_new_tag.emit(current_node))
                menu.addSeparator()
        except Exception:
            pass
        ...
        menu.addAction("✂️ 剪下", lambda: (lambda l=_collect_selected_tree_items(): self.clipboard_manager.cut(l))())
        menu.addAction("📋 複製", lambda: (lambda l=_collect_selected_tree_items(): self.clipboard_manager.copy(l))())

        # 貼上：目標為 effective_target（當表格有選取則優先使用代表性樹項）
        menu.addAction("📥 貼上", lambda: self.tree.request_paste_item.emit(effective_target))

        # 編輯 / 內容：為代表性樹項開啟內容編輯器
        if rep_tree_item is not None:
            menu.addAction("✏️ 內容", lambda: self.tree.request_edit_item.emit(rep_tree_item))
        else:
            menu.addAction("✏️ 內容")
        menu.addSeparator()
    ...
    def _copy_monitor_value(self, pos):
        ...
        try:
            ...
            if index.isValid():
                # 獲取值列 (通常是第3列)
                value_index = self.monitor_model.index(index.row(), 2)  # Column 2 = Value
    ...
    def _write_monitor_value(self, pos):
        ...
        try:
            ...
            current_value = self.monitor_model.data(self.monitor_model.index(index.row(), 3))  # Column 3 = Value
            
            print(f"[DEBUG] Write value for tag_path: {tag_path}, current_value: {current_value}")
            ...
            def on_write_requested(addr, fc, val):
                print(f"[UI] write_requested signal received: addr={addr}, fc={fc}, val={val}")
    ...
    def _build_tag_info_from_tree(self, tag_path):
        ...
        try:
            ...
            print(f"[DEBUG] _build_tag_info_from_tree: looking for tag '{tag_name_clean}' in path '{tag_path}'")
                        ...
                        def find_tag_recursive(item_list):
                            ...
                            for item in item_list:
                                if item.get('type') == 'Tag' and item.get('text') == tag_name_clean:
                                    ...
                                    print(f"[DEBUG] Found tag in config: {tag_name_clean}, device_id={device_id}")
                                    return tag_data
                ...
                if node.data(0, Qt.ItemDataRole.UserRole) == 'Tag':
                    ...
                    if node_text_clean == tag_name_clean:
                        ...
                        tag_data = {
                            ...
                            'data_type': self._safe_data(node, 2, 'Word'),  # 數據類型在角色 2
                            'read_write': self._safe_data(node, 3, 'Read/Write'),  # 讀寫在角色 3
                            'function_code': 16,  # 默認值
                            'device_id': 1,  # 默認值
                            'device_data_access': {'func_05': 1, 'func_06': 1}  # 默認值
                        }
                        
                        print(f"[DEBUG] Found tag in UI tree: {tag_name_clean}, address={tag_data.get('address')}, data_type={tag_data.get('data_type')}")
                                        ...
                                        for ch in self.project.get('channels', []):
                                            for dev in ch.get('children', []):
                                                if dev.get('text') == parent_text:
                                                    tag_data['device_id'] = dev.get('general', {}).get('device_id', 1)
                                                    tag_data['device_data_access'] = dev.get('data_access', {'func_05': 1, 'func_06': 1})
                                                    print(f"[DEBUG] Updated device_id={tag_data['device_id']}, data_access={tag_data['device_data_access']}")
                                                    break
                                except:
                                    pass
                                break
            ...
            def search_tree(node):
                ...
                try:
                    ...
                    if node_type == "Tag" and node_text == tag_name_clean:
                        ...
                        print(f"[DEBUG] Found tag node, tag_data type: {type(tag_data)}, value: {tag_data if not isinstance(tag_data, dict) else 'dict'}")
                        
                        # 確保返回字典
                        if isinstance(tag_data, dict):
                            return tag_data
                        else:
                            print(f"[ERROR] tag_data is not a dict, it's {type(tag_data)}")
                            return None
                except Exception as e:
    ...
    def _execute_tag_write(self, address, fc, value, tag_item):
        """執行標籤寫入操作"""
        print(f"[EXECUTE_WRITE] Called with: address={address}, fc={fc}, value={value}, tag_item keys={tag_item.keys() if isinstance(tag_item, dict) else 'N/A'}")
        try:
            # 檢查 Runtime 狀態
            if not hasattr(self, 'runtime_monitor') or not self.runtime_monitor:
                print(f"[ERROR] runtime_monitor not available")
                QMessageBox.warning(self, "警告", "運行時監視未啟動\n請先點擊 'Runtime' 按鈕啟動")
                return
            ...
            if not workers:
                ...
                QMessageBox.warning(self, "警告", "設備 worker 未初始化\n請確保 Runtime 已正確啟動並連接設備")
                return
            ...
            print(f"[DEBUG] device_id from tag_item: {device_id} (type: {type(device_id)})")
            ...
            for key, w in workers.items():
                ...
                if f"-Device{device_id}" in str(key) or f"Device{device_id}" in str(key):
                    worker = w
                    config_id = key
            ...
            if not worker:
                ...
                QMessageBox.warning(self, "警告", f"找不到設備的 worker\n可用設備: {available_keys}")
                return
            ...
            try:
                import json
                import io
                project_path = getattr(self, 'current_project_path', None) or 'Project.json'
                if not os.path.isabs(project_path):
                    project_path = os.path.join(os.path.dirname(__file__), project_path)
                if os.path.exists(project_path):
                    # 用 io.open 確保 UTF-8 編碼
                    with io.open(project_path, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    for channel in project_data.get('channels', []):
                        for device in channel.get('children', []):
                            if device.get('type') == 'Device':
                                ...
                                if str(dev_id_cfg) == device_id_str or dev_id_cfg == device_id:
            ...
            tag_info = {
                ...
                'device_id': device_id,
                'data_type': self._normalize_data_type(tag_item.get('data_type', 'uint16')),
                'byte_order': tag_item.get('byte_order') or mapped_encoding.get('byte_order'),
                'word_order': tag_item.get('word_order') or mapped_encoding.get('word_order'),
                'dword_order': tag_item.get('dword_order') or mapped_encoding.get('dword_order'),
                'bit_order': tag_item.get('bit_order') or mapped_encoding.get('bit_order'),
                'treat_longs_as_decimals': tag_item.get('treat_longs_as_decimals', False) or mapped_encoding.get('treat_longs_as_decimals', False),
            }
            ...
            if success:
                ...
                print(f"[UI] Write enqueued: addr={address} fc={selected_fc} value={value}")
            else:
    ...
    def _select_write_function_code(self, tag_item, value):
        ...
        try:
            ...
            print(f"[DEBUG] func_05_enabled: {func_05_enabled}, func_06_enabled: {func_06_enabled}")
            
            # 判斷數據類型是否需要多個 register
            needs_multiple = 'float' in data_type or 'double' in data_type or 'long' in data_type or 'dword' in data_type
            ...
            try:
                ...
                addr_category = int(addr_prefix)  # 如果是 "0005" 會變 5，如果是 "4000" 會變 4000
                print(f"[DEBUG] addr_str: {addr_str}, addr_prefix: {addr_prefix}, addr_category: {addr_category}")
            ...
            if addr_category < 1000:  # 0xxxx range (Coils)
                ...
                # 先檢查數據類型
                if needs_multiple:
                    print(f"[DEBUG] Selecting FC 16 (multiple registers needed for {data_type})")
                    return 16
                ...
                if func_06_enabled:
                    ...
                    print(f"[DEBUG] Selecting FC 16 (Holding Register, func_06 disabled)")
                    return 16
            ...
            try:
                ...
                for r in rows:
                    try:
                        ...
                        tag_tree_item = None
                        try:
                            tag_tree_item = self._safe_data(itm, Qt.ItemDataRole.UserRole)
                        except Exception:
                            tag_tree_item = None
    ...
    def _handle_shortcut(self, action):
        ...
        is_table = fw is self.tag_table or (hasattr(fw, 'parent') and getattr(fw, 'parent') is self.tag_table)
        if is_table:
            if action == 'copy':
                ...
                items = self._collect_selected_tree_items()
                if items:
                    parent = self.clipboard_manager.cut(items if len(items) > 1 else items[0])
                    if parent:
                        try:
                            self.update_right_table(parent, 0)
                        except Exception:
    ...
    def on_new_channel(self, parent_item):
        ...
        try:
            import re
            existing_names = [parent_item.child(i).text(0) for i in range(parent_item.childCount())]
            nums = []
        ...
        if dialog.exec():
            ...
            try:
                ...
                try:
                        QTimer.singleShot(150, lambda ni=new_item: (self.tree.setCurrentItem(ni), self.update_right_table(ni, 0)))
                except Exception:
                    pass
            except Exception:
                try:
    ...
    def on_new_device(self, channel_item):
        ...
        try:
            import re
            existing_names = [channel_item.child(i).text(0) for i in range(channel_item.childCount())]
            nums = []
            for n in existing_names:
                try:
        ...
        dialog = DeviceDialog(self, suggested_name=suggested_name, driver_type=driver_name)
        # load_data 接受巢狀的 'general' 結構；為一致性使用巢狀格式
        dialog.load_data({"general": {"name": suggested_name, "device_id": next_id, "description": ""}})
        if dialog.exec():
            data = dialog.get_data()
        ...
        if dlg.exec():
            ...
            try:
                if getattr(self, 'controller', None):
                    ...
                    new_item.setText(0, (data.get("general") or {}).get("name") or data.get("name") or suggested_name)
                    new_item.setData(1, Qt.ItemDataRole.UserRole, (data.get("general") or {}).get("description") or data.get("description") or "")
            except Exception:
                try:
                    new_item.setText(0, (data.get("general") or {}).get("name") or data.get("name") or suggested_name)
                    new_item.setData(1, Qt.ItemDataRole.UserRole, (data.get("general") or {}).get("description") or data.get("description") or "")
    ...
    def on_new_tag(self, parent_item):
        ...
        try:
            try:
                ptype = self._safe_data(parent_item, 0, None) if parent_item is not None else None
        ...
        try:
            if getattr(self, 'controller', None):
                try:
                    ...
                    res = self._call_controller('calculate_next_address', parent_item, prefix=prefix, new_type=new_type)
        ...
        try:
            ...
            dlg = TagDialog(self, suggested_name=suggested_name, suggested_addr=suggested_addr, target_item=parent_item, is_new=True)
            # 載入最小預設值
            try:
                dlg.load_data({"general": {"name": suggested_name, "address": suggested_addr or "", "data_type": "Word", "description": ""}, "scaling": {"type": "None"}})
            except Exception:
                pass
            if dlg.exec():
                new_data = dlg.get_data()
                # 驗證是否有重複項目
                new_name = (new_data.get('general') or {}).get('name') or new_data.get('name')
                new_addr = (new_data.get('general') or {}).get('address') or new_data.get('address')
                if new_name in used_names:
                    QMessageBox.warning(self, '新增 Tag 失敗', f'名稱 {new_name} 已存在。')
                    return
                if new_addr in used_addresses:
                    QMessageBox.warning(self, '新增 Tag 失敗', f'位址 {new_addr} 已存在。')
                    return
                ...
                try:
                    if getattr(self, 'controller', None):
                        ...
                        try:
                            new_item.setText(0, (new_data.get('general') or {}).get('name') or new_data.get('name') or suggested_name)
                        except Exception:
                            pass
                        try:
                            new_item.setData(1, Qt.ItemDataRole.UserRole, (new_data.get('general') or {}).get('address') or new_data.get('address'))
                        except Exception:
                            pass
                except Exception:
                    try:
                ...
                if node_type == 'Group':
                    try:
                        ...
                        if dlg.exec():
                            ...
                            try:
                                if _prev_sizes is not None and not getattr(self, '_splitter_user_moved', False):
                                    self._set_splitter_sizes(_prev_sizes, reason='restore_on_show_content')
                            except Exception:
                                pass
                        try:
            ...
            try:
                ...
                try:
                    ...
                    if not getattr(self, '_splitter_initialized', False):
                        ...
                        try:
                            self._set_splitter_sizes([left_w, right_w], reason='update_right_table_initial_half')
                        except Exception:
                            pass
                        self._splitter_initialized = True
                except Exception:
                ...
                try:
                    ...
                    if self.tag_table.rowCount() > 0 and self.tag_table.columnCount() > 1:
    ...
    def update_right_table(self, item, _column=0):
        ...
        if node_type == "Connectivity":
            ...
            debug_startup = (hasattr(self, 'tree') and item is getattr(self.tree, 'conn_node', None))
            for i in range(item.childCount()):
                child = item.child(i)
            ...
            headers = ["Device Name", "Unit ID", "Timing", "DataAccess", "DataEncoding", "Block Sizes"]
            ...
            for i in range(item.childCount()):
                ...
                try:
                    ...
                    if isinstance(timing, dict):
                        if 'rtu over tcp' in drv_type:
                            vals = [
                                timing.get('connect_timeout') or timing.get('req_timeout') or '',
                                timing.get('connect_attempts') or timing.get('attempts') or timing.get('attempts_before_timeout') or '',
                                timing.get('request_timeout') or timing.get('req_timeout') or '',
                                timing.get('attempts_before_timeout') or timing.get('attempts') or '',
                                timing.get('inter_request_delay') or timing.get('inter_req_delay') or '',
                            ]
                        ...
                        timing_str = ",".join([str(x) for x in vals if x is not None and str(x) != ""])
                    else:
                ...
                try:
                    ...
                    if isinstance(access, dict):
                        access_keys = ["zero_based", "zero_based_bit", "bit_writes", "func_06", "func_05"]
                        vals = []
                ...
                try:
                    ...
                    if isinstance(blocks, dict):
                        ...
                        block_vals = [str(blocks.get(k)) for k in block_keys if blocks.get(k) is not None]
        ...
        try:
            ...
            logging.error(f"Unexpected error updating monitor table: {e}", exc_info=True)
    ...
    def _append_tag_row(self, table, t):
        ...
        try:
            ...
            scale_type = scaling_data.get("type", "None") if isinstance(scaling_data, dict) else str(scaling_data or "")
        except Exception:
            scaling_data = {}
            scale_type = ""
        ...
        rl = scaling_data.get("raw_low", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(6, rl)
        rh = scaling_data.get("raw_high", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(7, rh)
        sdt = scaling_data.get("scaled_type", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(8, sdt)
        sl = scaling_data.get("scaled_low", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(9, sl)
        sh = scaling_data.get("scaled_high", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(10, sh)
        cl = scaling_data.get("clamp_low", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(11, cl)
        ch = scaling_data.get("clamp_high", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(12, ch)
        units = scaling_data.get("units", "") if isinstance(scaling_data, dict) and show_scaling else ""
        _mk(13, units)
    ...
    def _append_channel_row(self, table, ch):
        ...
        try:
            ...
            serial_like = False
            if ("serial" in drv_low) or ("rtu" in drv_low and "over tcp" not in drv_low):
                serial_like = True
            if serial_like:
                ...
                adapter_ip = params.get('network_adapter_ip') or params.get('adapter_ip') or params.get('ip')
                adapter = params.get('network_adapter') or params.get('adapter') or params.get('adapter_name')
                if adapter:
                    conn = str(adapter)
                elif ip and port:
                    conn = f"{ip}:{port}"
        ...
        try:
            ...
            try:
                if serial_like:
                    ...
                    nap = params.get('network_adapter_ip') or params.get('adapter_ip') or params.get('ip')
                    if nap:
                        param_str = str(nap)
                    else:
                        parts = []
    ...
    def _collect_tags_from_item(self, item, include_path=True, ancestors=None):
        ...
        if node_type == "Tag":
            ...
            for i in range(item.childCount()):
                try:
                    child = item.child(i)
                    results.extend(self._collect_tags_from_item(child, include_path, current_ancestors))
                except Exception:
                    continue
    ...
    def _update_monitor_table(self, item):
        ...
        for tag_item, tag_path in tags:
            ...
            try:
                ...
                dtype = re.sub(r"\(Array\)|\bArray\b|\[\]|\s*\[\s*\d+\s*\]", "", str(dtype_display), flags=re.IGNORECASE).strip()
                ...
                if not is_array:
                    ...
                    is_array = isinstance(addr, str) and re.search(r"\[\s*\d+\s*\]", addr)
        ...
        if hasattr(self, 'monitor_model') and self.monitor_model:
            ...
            logging.info(f"[MONITOR_TABLE] Loaded {len(model_tags)} tags for display (virtual scrolling)")
            # DEBUG: show first few tag paths
            for i, (path, dtype, access) in enumerate(model_tags[:5]):
                logging.debug(f"[MONITOR_TABLE] Tag[{i}]: {path}")
            if len(model_tags) > 5:
                logging.debug(f"[MONITOR_TABLE] ... and {len(model_tags) - 5} more tags")
    ...
    def open_project(self):
        ...
        try:
            ...
            try:
                QTimer.singleShot(0, lambda: self.update_right_table(self.tree.conn_node, 0))
                QTimer.singleShot(250, lambda: self.update_right_table(self.tree.conn_node, 0))
                QTimer.singleShot(1000, lambda: self.update_right_table(self.tree.conn_node, 0))
            except Exception:
                try:
                    self.update_right_table(self.tree.conn_node, 0)
                except Exception:
    ...
    def start_runtime_polling(self):
        ...
        try:
            ...
            if not hasattr(self, 'modbus_data_buffer') or self.modbus_data_buffer is None:
            ...
            if self.runtime_monitor is None:
                self.runtime_monitor = RuntimeMonitor(tree_root_item=self.tree.conn_node, signals_instance=self.runtime_signals)
                logging.info(f"Created RuntimeMonitor with signals instance")
                
                # Connect signals only once during creation
                try:
                    self.runtime_monitor.signal_tag_updated.connect(self._on_runtime_tag_updated)
                ...
                try:
                    self.runtime_monitor.signal_stopped.connect(self._on_runtime_stopped)
                ...
                logging.info("✓ All callbacks marked as connected, ready to receive signals")
            else:
                logging.info("✓ RuntimeMonitor already created, reusing existing instance")
    ...
    def _on_monitor_scroll(self, value):
        ...
        if hasattr(self, 'monitor_model') and self.monitor_model:
            ...
            if first_visible_index.isValid():
                ...
                first_visible_row = 0
            logging.info(f"[SCROLL_EVENT] value={value}, first_visible_row={first_visible_row}")
            self.monitor_model.update_visible_rows(first_visible_row)
    
    def _on_runtime_tag_updated(self, tag_name: str, value, timestamp: float, quality: str, update_count: int):
        """Modbus worker callback - write data to buffer, update model."""
        try:
            # Write to central data buffer (not directly to UI)
            if hasattr(self, 'modbus_data_buffer') and self.modbus_data_buffer:
                self.modbus_data_buffer.update_tag_value(tag_name, value, timestamp, quality, update_count)
                ...
                if hasattr(self, 'monitor_model') and self.monitor_model:
                    ...
                    self.monitor_model.update_tag_value(tag_name, value, timestamp_str, quality, update_count)
        
        except Exception as e:
            logging.error(f"Error in _on_runtime_tag_updated callback: {tag_name}: {e}", exc_info=True)
